...
// DB returns a pointer to the suite's test Database, named "stork-test", initializing a new Database, Session, and/or
...
func (suite *MongoSuite) DB() *mgo.Database {
	...
	if suite.db == nil {
		suite.db = suite.DBServer().Session().DB("stork-test")
	}
	return suite.db
}
